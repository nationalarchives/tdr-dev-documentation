...
* `git-secrets --add --global -a '([^0-9])*123456789012([^0-9])*'`: this will add an allowed dummy AWS account number of *012345678912*. This can be used, if for example, when writing example code in a README which uses an AWS account number.
